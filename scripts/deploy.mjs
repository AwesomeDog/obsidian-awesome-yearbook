...
if (!vault) {
  ...
  const target = path.join(vault, ".obsidian", "plugins", "awesome-yearbook");
  ...
  try {
    ...
    console.log(`Installed Awesome Yearbook in ${target}`);
  } catch (error) {
    console.error(`Unable to deploy Awesome Yearbook to ${target}.`);
